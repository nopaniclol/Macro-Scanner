            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    if 'Close' in df.columns:
                        ...
                        price_series = df.iloc[:, 1] if df.shape[1] > 1 else df.iloc[:, 0]

                    # Set index to Date if available
                    if 'Date' in df.columns:
                        price_series = pd.Series(price_series.values, index=pd.to_datetime(df['Date']))
        ...
        try:
            ...
            if vix_df is not None and not vix_df.empty:
                ...
                if 'Close' in vix_df.columns:
                    vix_col = vix_df['Close']
                elif 'close' in vix_df.columns:
                    vix_col = vix_df['close']
                else:
                    vix_col = vix_df.iloc[:, 1] if vix_df.shape[1] > 1 else vix_df.iloc[:, 0]

                # Validate VIX is numeric
                if pd.api.types.is_numeric_dtype(vix_col):
                    # Set index to Date if available
                    if 'Date' in vix_df.columns:
                        vix_col = pd.Series(vix_col.values, index=pd.to_datetime(vix_df['Date']))
                    prices['VIX'] = vix_col
