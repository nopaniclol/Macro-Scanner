...
# Which CFTC level score feeds the cftc_positioning composite.
#   "full"    -- percentile against all history since CFTC_HISTORY_START.
#                This is what the Spectra explainer describes ("looking back
#                across the sample from today back to January 1, 2000").
#   "rolling" -- percentile against a ROLLING_PERCENTILE_YEARS window.
#                More responsive to regime change, less comparable over time.
# Both are always computed and reported; this only picks which one aggregates.
# NOTE: with CFTC_HISTORY_START at 2010, "full" is a ~15-year baseline, not the
# 25-year one the explainer assumes.
CFTC_LEVEL_MODE = "rolling"

# Staleness guard. If a Bloomberg ticker quietly stops updating, clean_series
# drops the NaNs and the last good value scores as though it were current --
# a plausible-looking number with no sign it is weeks old. Any series whose
# last observation is older than this many business days gets flagged.
# CFTC series update weekly, so the threshold has to clear a normal gap.
STALENESS_MAX_BUSINESS_DAYS = 10

# -----------------------------------------------------------------------------
# USD PROXY
# -----------------------------------------------------------------------------
# There is no CFTC contract for the dollar index, so the explainer builds USD
# as "the inverted average of all scores vs. each currency". We do the same,
# but sign each leg explicitly rather than blanket-inverting, because our pairs
# are not all quoted the same way round:
#
#   EUR/USD, GBP/USD, AUD/USD -> a positive score is long the foreign currency,
#                                 i.e. SHORT dollars               -> sign -1
#   USD/JPY                    -> cftc_sign already flipped this to read in
#                                 USD terms, so it is already long USD -> +1
#
# Blanket-inverting would double-count the JPY flip and put that leg the wrong
# way round. Set USD_PROXY_SYMBOL to None to disable the derivation entirely.

USD_PROXY_SYMBOL = "DXY"
...
# Positioning fields derived for the USD proxy. Raw quote columns are excluded
# deliberately: averaging raw vol levels across different pairs is meaningless.
USD_DERIVED_FIELDS = (
    "cftc_level_rolling",
    "cftc_level_full",
    "cftc_4wk_chg",
...
# Last observation date seen for each ticker this session, used by the
# staleness check. Populated by fetch_series_bql.
_LAST_OBSERVATION: Dict[str, pd.Timestamp] = {}


def stale_series(max_business_days: int = STALENESS_MAX_BUSINESS_DAYS) -> pd.DataFrame:
    """
    Tickers fetched this session whose most recent observation is older than
    the threshold, most stale first.
    """
    if not _LAST_OBSERVATION:
        return pd.DataFrame()

    today = pd.Timestamp(date.today())
    rows = []
    for ticker, last_date in _LAST_OBSERVATION.items():
        age = len(pd.bdate_range(last_date, today)) - 1
    ...
    return (
        pd.DataFrame(rows)
        .sort_values("business_days_old", ascending=False)
        .reset_index(drop=True)
    )


def report_stale_series(
    max_business_days: int = STALENESS_MAX_BUSINESS_DAYS,
    max_listed: int = 10,
) -> None:
    """
    Print any stale inputs. Silence here means every series is current.

    Only the worst offenders are listed: if the whole feed is behind (a dead
    connection, or running before the day's data lands) a full listing is a
    wall of text that buries the point. Use stale_series() for the full frame.
    """
    stale = stale_series(max_business_days)
    total = len(_LAST_OBSERVATION)

    print(f"--- Stale inputs (last observation > {max_business_days} business days old) ---")
    if stale.empty:
        print(f"  None. All {total} fetched series are current.")
        return

    if len(stale) == total and total > 1:
        print(f"  ALL {total} series are stale -- this looks like a feed or date")
        print("  problem rather than individual tickers going quiet.")

    for _, row in stale.head(max_listed).iterrows():
        print(
            f"  {row['ticker']:<28} last={row['last_observation']} "
            f"({row['business_days_old']} business days old)"
        )

    if len(stale) > max_listed:
        print(f"  ... and {len(stale) - max_listed} more. See stale_series() for the full list.")

    print(f"  {len(stale)} of {total} series carrying forward a stale value.")
...
def fetch_series_bql(ticker: str, start_date: str) -> pd.Series:
    ...
    if not series.empty:
        _LAST_OBSERVATION[ticker] = series.index.max()
...
def build_positioning_history(symbol: str, instrument: dict) -> Tuple[pd.DataFrame, dict]:
    ...
    level_column = (
        "cftc_level_full" if CFTC_LEVEL_MODE == "full" else "cftc_level_rolling"
    )
    history["cftc_positioning"] = history[
        [level_column, "cftc_4wk_chg"]
    ].mean(axis=1)
...
def fetch_risk_reversal_history(symbol: str, instrument: dict) -> Tuple[pd.DataFrame, dict]:
    ...
    for tenor, key in (("1M", "rr_1m"), ("6M", "rr_6m")):
        ...
        try:
            ...
            # Raw quote alongside the score, so a +/-10 (capped) reading can be
            # sanity-checked against the actual vol level without a re-fetch.
            summary[f"{key}_raw"] = _round_or_none(raw.iloc[-1]) if len(raw) else None
        except Exception as exc:
...
# =============================================================================
# PLOT LABELLING HELPERS
# =============================================================================

# Human-readable names for the score columns, used in plot legends so a reader
# does not have to know what "dev_20d" or "rr_1m" mean.
SCORE_DISPLAY_NAMES = {
    "POSITIONING": "Positioning (composite)",
    "cftc_positioning": "CFTC positioning",
    "options_skew": "Options skew (risk reversals)",
    "cftc_level_rolling": "CFTC level (rolling %ile)",
    "cftc_level_full": "CFTC level (full-sample %ile)",
    "cftc_4wk_chg": "CFTC 4-week change (z)",
    "rr_1m": "Risk reversal 1M (z)",
    "rr_6m": "Risk reversal 6M (z)",
    "MOMENTUM": "Momentum (composite)",
    "rsi": "RSI (14d)",
    "rsi_raw": "RSI (14d, raw 0-100)",
    "dev_20d": "Deviation from 20d MA (z)",
    "dev_100d": "Deviation from 100d MA (z)",
}

# Every score on these axes is a capped z-score: +/-3 standard deviations is
# rescaled to +/-10, so a reading of exactly +/-10 means the cap was hit, not
# that the underlying quote reached 10 of anything.
SCORE_AXIS_LABEL = "Score (z-scored, -10 to +10)"
SCORE_SCALE_NOTE = "+/-10 = 3 standard deviation cap    +/-5 = extreme threshold"


def display_name(column: str) -> str:
    """Readable label for a score column, falling back to the raw name."""
    return SCORE_DISPLAY_NAMES.get(column, column)


def label_time_axis(axis) -> None:
    """
    Label the x-axis by year, choosing a tick spacing that suits the span so a
    20-year history does not print an unreadable wall of dates.
    """
    ...
    try:
        ...
        span_years = (end - start) / 365.25
    except Exception:
        return

    if span_years > 1.5:
        step = max(1, int(span_years // 12) + 1)
        axis.xaxis.set_major_locator(mdates.YearLocator(step))
...
def plot_rsi_heatmap(frame: pd.DataFrame, ncols: int = 5, cell_size=(1.9, 1.3)) -> None:
    ...
    axis.set_title(
        "Tracked-Instrument RSI Heatmap\nCurrent 14-day RSI (sorted high to low)",
        fontsize=13, fontweight="bold",
    )

    # Colour key: without it the shading carries no stated meaning.
    colour_bar = axis.figure.colorbar(
        plt.cm.ScalarMappable(norm=normalise, cmap=colour_map),
        ax=axis, orientation="horizontal",
        fraction=0.05, pad=0.04, aspect=40,
    )
    colour_bar.set_label(
        "14-day RSI    (0 = oversold, 50 = neutral, 100 = overbought)",
        fontsize=8,
    )
    colour_bar.set_ticks([0, 30, 50, 70, 100])
...
def plot_score_history(
    ...
    for column in series.columns:
        axis.plot(
            series.index, series[column],
            marker="o", markersize=3, label=display_name(column),
        )
    ...
    axis.text(
        0.995, 5, "+5 extreme", transform=axis.get_yaxis_transform(),
    ...
    axis.set_yticks(range(-10, 11, 5))
    axis.set_ylabel(SCORE_AXIS_LABEL)
    label_time_axis(axis)
...
# =============================================================================
# USD PROXY DERIVATION
# =============================================================================

def derive_usd_positioning(report: pd.DataFrame) -> pd.DataFrame:
    """
    Fill the USD proxy row's positioning scores from the currency legs.

    The dollar index has no CFTC contract, so without this the DXY row carries
    no positioning at all. Momentum is left untouched -- that comes from the
    actual DXY price series and is already correct.

    Legs that are missing simply drop out of the average, and the number of
    legs actually used is recorded so a thin average is visible rather than
    silently presented as though it used the full basket.
    """
    symbol = USD_PROXY_SYMBOL

    if not symbol or symbol not in report.index:
        return report

    report = report.copy()
    legs = {s: w for s, w in USD_BASKET_SIGNS.items() if s in report.index}

    if not legs:
        print(f"  [warn] No currency legs available to derive {symbol} positioning.")
        return report

    leg_counts = []
    for field in USD_DERIVED_FIELDS:
        if field not in report.columns:
            continue

        values = [
            sign * float(report.at[leg, field])
            for leg, sign in legs.items()
            if pd.notna(report.at[leg, field])
        ]
        report.at[symbol, field] = round(sum(values) / len(values), 2) if values else None
        leg_counts.append(len(values))

    report.at[symbol, "cftc_metric"] = f"derived from {len(legs)} FX legs"

    report.at[symbol, "POSITIONING"] = weighted_mean(
        {
            "cftc_positioning": report.at[symbol, "cftc_positioning"],
            "options_skew": report.at[symbol, "options_skew"],
        },
        POSITIONING_WEIGHTS,
        MIN_POSITIONING_COMPONENTS,
    )
    report.at[symbol, "positioning_component_count"] = available_count(
        [report.at[symbol, "cftc_positioning"], report.at[symbol, "options_skew"]]
    )
    report.at[symbol, "positioning_interpretation"] = concentration_label(
        report.at[symbol, "POSITIONING"]
    )
    report.at[symbol, "skew_interpretation"] = skew_label(
        report.at[symbol, "options_skew"]
    )

    print(
        f"  Derived {symbol} positioning from "
        f"{', '.join(f'{s}({w:+.0f})' for s, w in legs.items())}"
    )
    return report
...
REPORT_COLUMNS = [
    ...
    "cftc_4wk_chg",
    "options_skew", "rr_component_count",
    "rr_1m", "rr_1m_raw", "rr_6m", "rr_6m_raw", "skew_interpretation",
...
def plot_instrument_detail(
    ...
    for axis in (
        ...
        # name the reference lines so they are self-explanatory
        axis.text(
            0.995, 5, "+5 extreme",
    ...
    # Only the bottom axis carries the x label (sharex=True).
    label_time_axis(momentum_axis)
    price_axis.set_xlabel("")
    positioning_axis.set_xlabel("")
...
def main() -> None:
    ...
    report = derive_usd_positioning(report)
